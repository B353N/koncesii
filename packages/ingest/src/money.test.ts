...
test("процент не е сума: полето е липсващо, суровият текст остава", () => {
  for (const raw of [
    "относителна тежест 80%; 2) Критерий:",
    "Относителна тежест на критерия в оценката на офертите -100%",
    "Периодични парични възнаграждения-4% от реализираните приходи",
    "Годишно: 5 % от общия размер на приходите, но не по-малко от 5807 лв.",
  ]) {
    const p = parseMoney(raw);
    expect(p).toMatchObject({ raw, value: null, eur: null, flag: "missing" });
  }
  // процент след сумата не пречи
  expect(parseMoney("1 200 лв., 20% ДДС").value).toBe(1200);
});
